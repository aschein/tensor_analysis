...
#subset that we want (all RUID's where MAP_CHANGE is not NaN
all_finite_ruid_in_MAP_CHANGE = np.sort(pd.unique(df_MAP_CHANGE_finite['RUID']))

#grab parts of df's corredsponding to RUID we want (all the ones where MAP change is finite)
df_med_all_finite = df_med[df_med.RUID.isin(all_finite_ruid_in_MAP_CHANGE)]
df_jd_all_finite = df_jd[df_jd['RUID'].isin(all_finite_ruid_in_MAP_CHANGE)]
df_jdrange_all_finite = df_jdrange[df_jdrange['RUID'].isin(all_finite_ruid_in_MAP_CHANGE)]
df_MAP_CHANGE_all_finite = df_MAP_CHANGE_finite[df_MAP_CHANGE_finite['RUID'].isin(all_finite_ruid_in_MAP_CHANGE)]
df_med_jdrange_all_finite = pd.merge(df_med_all_finite, df_jdrange_all_finite, left_on="RUID", right_on="RUID")
df_med_jdrange_all_finite = df_med_jdrange_all_finite.fillna(0)

##re-sort the df's so that they are sorted by RUID (all of them should be ordered the same way by RUID)
df_MAP_CHANGE_all_finite = df_MAP_CHANGE_all_finite.sort(['RUID'], ascending=1)
df_med_jdrange_all_finite = df_med_jdrange_all_finite.sort(['RUID'], ascending=1)
df_med_all_finite = df_med_jdrange_all_finite.iloc[:,0:14] #just emds
cols_for_jdrange = [1] + range(15,202)
cols_for_med = range(14)
df_jdrange_all_finite = df_med_jdrange_all_finite.iloc[:,cols_for_jdrange] #just jdranges

#convert to np arrays
df_med_cols = [col for col in df_med_all_finite if col not in ['RUID']] #column names for meds
df_jd_cols = [col for col in df_jd_all_finite if col not in ['RUID']] #column names for jd codes
df_jdrange_cols = [col for col in df_jdrange_all_finite if col not in ['RUID']] #column names for jd ranges
df_med_test_data = df_med_all_finite[df_med_cols]
df_jd_test_data = df_jd_all_finite[df_jd_cols]
df_jdrange_test_data = df_jdrange_all_finite[df_jdrange_cols]
...
all_finite_ruid_in_med_and_jdrange = np.sort(pd.unique(df_med_jdrange_all_finite['RUID']))

#start with list of lists
d_ruid_matrixMedJd = dict()
l_data_by_pt = []
l_data_pt_med_jdrange = []
for ruid_index in range(len(all_finite_ruid_in_med_and_jdrange)):
...
for pt in all_finite_ruid_in_med_and_jdrange:
...
df_MAP_CHANGE_all_finite_ruid_in_testset = df_MAP_CHANGE_all_finite[df_MAP_CHANGE_all_finite['RUID'].isin(all_finite_ruid_in_med_and_jdrange)]
df_MAP_CHANGE_all_finite_ruid_in_testset['MAP_CHANGE_GOOD'] = df_MAP_CHANGE_all_finite_ruid_in_testset['MEDIAN_MAP_CHANGE']<=-2
df_MAP_CHANGE_all_finite_ruid_in_testset['MAP_CHANGE_GOOD'] = df_MAP_CHANGE_all_finite_ruid_in_testset['MAP_CHANGE_GOOD'].astype('int')
l_patClass = df_MAP_CHANGE_all_finite_ruid_in_testset['MAP_CHANGE_GOOD']
od_patClass_all_finite = OrderedDict(zip(patDict.keys(), l_patClass))
...
Yinfo_all_finite = spntf_htn_all_finite.computeDecomp()

marbleElapse = time.time() - startTime #elapsed time
#marbleFMS, marbleFOS, marbleNNZ = calculateValues(loaded_X, spntf_htn_all_finite.M[SP_NTF.REG_LOCATION]) #this is wrong - fix it
...
#pftMat_htn_all_finite, pftBias_htn_all_finite = spntf_htn_all_finite.projectData(spntf_htn_all_finite, 0, maxinner=innerIter)
#	
